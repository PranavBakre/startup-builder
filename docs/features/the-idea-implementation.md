...
Code is split across 6 scripts with clear responsibilities:

| Script | Type | Role |
|---|---|---|
| `world.gd` | Node2D | Orchestrator — spawning, movement/collision, NPC interaction, input routing |
| `map_generator.gd` | class (static) | Pure computation — `MapGenerator.generate(w, h)` returns `{tile_map, buildings}` |
| `tile_renderer.gd` | class (static) | `TileRenderer.render(...)` creates Sprite2D nodes from map data |
| `dialogue_manager.gd` | Node | Dialogue state, typewriter effect, `start(npc, lines)` / `advance()`. Emits `dialogue_ended(npc)` |
| `camera_controller.gd` | Camera2D | `follow(target_pos, delta)` + `handle_input(event)` for zoom |
| `player.gd` / `npc.gd` | Node2D / Area2D | Player movement, NPC data storage |
...
└── CanvasLayer
    ├── DialogueBox (Panel)
    │   └── MarginContainer/VBoxContainer/{NPCName, DialogueText}
    └── InteractPrompt (Label)
```

### Dialogue system (dialogue_manager.gd)

- **State:** `active_npc`, `dialogue_lines`, `line_index`, `char_index`, `_timer` — all internal to DialogueManager
- **API:** `start(npc, lines)`, `advance()`, `process_typewriter(delta)`, `is_active()`
- **Signal:** `dialogue_ended(npc)` — world.gd connects to this for post-dialogue hooks
- **Flow:** world.gd calls `dialogue_manager.start(npc, lines)` → typewriter driven by `process_typewriter()` → `advance()` on input → emits `dialogue_ended(npc)`
- **Input:** world.gd checks `dialogue_manager.is_active()` to route to dialogue mode
- **Key design:** world.gd decides *which lines* to pass — DialogueManager doesn't know about re-talk/post-founding logic
- **UI refs:** `dialogue_box`, `npc_name_label`, `dialogue_text_label` — set by world.gd in `_ready()`
...
- **No game state singleton** — game state lives in world.gd
- **No `talked_to` tracking** — dialogue replays identically every time
- **`dialogue_ended` signal is the hook point** — world.gd's `_on_dialogue_ended(npc)` currently just re-checks NPC proximity; v0.2 will add problem collection here
...
1. If `dialogue_manager.is_active()` → dialogue mode (existing)
...
**File:** `game/scripts/world.gd`
**Function:** `_on_dialogue_ended()` (signal callback, already connected in `_ready()`)

Replace the current `_on_dialogue_ended()`:
...
**File:** `game/scripts/world.gd`
**Function:** `_try_interact()`

Replace current `_try_interact()` to pick lines based on `talked_to` state:
...
No other changes needed — DialogueManager already accepts lines as an argument to `start()`, so the typewriter, advance, and end logic all work unchanged. The old `active_dialogue_lines` refactor (touching `_process()` and `_advance_dialogue()`) is **eliminated** by the refactored architecture.
...
func _input(event):
	...
	# ... rest of existing input (interact, camera zoom)
```

**Important:** The existing dialogue check (`dialogue_manager.is_active()`) and everything after it stays the same. We're inserting the journal checks at the top.

Also block movement when journal is open. In `_process()`, change the movement condition:

```
OLD: if not dialogue_manager.is_active() and player and not player.is_moving:
NEW: if not dialogue_manager.is_active() and not journal_open and player and not player.is_moving:
```
...
| File                    | Changes                                                                                                                                                                                                                                            |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `game/scripts/world.gd` | Add state vars, modify `_try_interact()`, `_on_dialogue_ended()`, `_process()`, `_input()`. Add `_create_journal_ui()`, `_create_hud()`, `_open_journal()`, `_close_journal()`, `_populate_journal()`, `_on_found_company_pressed()`, `_on_company_name_submitted()`, `_update_hud()` |

No new files. No scene tree changes (all UI created in code). DialogueManager untouched — world.gd just passes different lines.
...
All dialogue layer logic lives in `_try_interact()` — DialogueManager just plays whatever lines it receives.
...
| File                    | Changes                                                                                                                                      |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `game/scripts/world.gd` | Add follow_up/post_founding to npc_data, restyle journal, add founded_label, update `_try_interact()` layering, add `_on_name_input_gui()` |
...
Call `show_checkmark()` when a problem is collected. In `_on_dialogue_ended()`, after adding to `discovered_problems`:
...
2. **Journal UI created in code** — No visual editor preview. Layout bugs won't be visible until runtime. **Mitigation:** Test at multiple window sizes. Consider moving to .tscn in a polish pass.

3. **Checkmark positioning** — The `Label` at `position = Vector2(-20, -TILE_SIZE * 0.7)` may look wrong at different zoom levels since it's a child of the NPC (world-space). **Mitigation:** Test at zoom 0.1, 0.5, 1.0.

4. **`_populate_journal()` rebuilds on every open** — `queue_free()` on old children is async. If journal is opened rapidly, stale children may appear briefly. **Mitigation:** Low risk with 5 items max. If flickering occurs, use `free()` instead or maintain children.

### Low Risk

5. **Button `pressed` signal with `bind()`** — Standard Godot pattern, but buttons are recreated each journal open. No cleanup needed since `queue_free()` handles disconnection.

6. **No save/load** — Game state resets on restart. Acceptable for v0.2; v0.3 addresses this.

### Eliminated risks (from pre-refactor)

- ~~**`active_dialogue_lines` refactor**~~ — DialogueManager accepts lines as an argument to `start()`, so world.gd never touches the dialogue loop internals. The typewriter/advance/end logic is untouched by v0.2 changes.
...
- 2026-02-15: Updated for architecture refactor — world.gd split into MapGenerator, TileRenderer, DialogueManager, CameraController. Dialogue line selection moved to `_try_interact()`, `_end_dialogue()` references → `_on_dialogue_ended()`, `active_dialogue_lines` refactor eliminated
